...
function getLinkText(l) {
    return HoverText(l.source) + "<br/>" + HoverText(l.target);
...
function depth2color(depth, level){
    if(depth == 0){
        return "gold";
    }
    else if(depth == 1){
...
function simulationUpdate() {
    ...
    nodes.forEach(function (d) {
        if( (nodeSwitch == 0) || (nodeSwitch == 1 && d.depth < 2) || (nodeSwitch == 2 && d.depth == 2)){
            ...
            context.fillStyle = depth2color(d.depth, d.level);
            context.fill();
            // context.fillText(d.level == 0 ? d.count : "", d.x, d.y);
    ...
    nodes.forEach(function (d) {
        if( (labelSwitch == 0) || (labelSwitch == 1 && d.depth < 2) || (labelSwitch == 2 && d.depth == 2)){
            ...
            context.fillText(d.level== 0 ? d.label + " ("+d.count+")" : d.label, d.x+15, d.y+4);
...
function HoverText(node){
    if(node.level == 0){ //Artist
        return "👤  " + node.label + "<br/>("+ node.count +" songs)";
    } else{
        return "🎵  " + node.label;
    }
}
...
d3.json("/Graph/artistsonglink", function (error, data) {
    mainf(data);
});